...
base = get_my_sigmoid_base(2, 0.6)
"""
2-gram 5
345gram 20
25
...
def task_construct_sentence(a_stn, data, syllables_l, max_consider_num,
    ...
    for i, syllable_prob in enumerate(syllables_l):  # the main reason to construct all possibles
        ...
        a_stn[1][2] = (prev_avg_ranking * (length - 1) + i) / length
        alpha = a_stn[1][3] = (a_stn[1][3] * (length - 1) + s) / length
        alpha = 10 - a_stn[1][2] / group
        ...
        for p_word, weight in zip(p_words, weights):  # constant time 1 - 3 times
            ...
            relation_intensity = gate * pow(sentence[1][0], power) + \
                                 is_connected * s * pow(weight, 0.5)
            ...
            score += min(729., pow(max(s, alpha), 3))
...
def make_possible_sentence(syllables_ls, data,
    ...
    for i in range(1, sentence_len):
        ...
        if k_round_filter or len(all_sentences) >= 3000:
            '''
            # tricky
