...
from phoneme import zelements, optionals, zindexes, mmap
    ...
    *_, settings = get_settings()
    ...
    def prepare_processing_graph(self, model_settings):
        ...
        with g.as_default():
            ...
            self.mfcc_ = my_signal.spectrogram(background_clamp,
                                               model_settings['window_size_samples'],
                                               model_settings['window_stride_samples'],
                                               model_settings['window_size_samples']) / (model_settings['window_size_samples'] / 2)
    ...
    def get_data(self, how_many=100, offset=0, background_frequency=0.0,
        ...
        targets = np.zeros((sample_count, model_settings['fingerprint_size']))
        desired_samples = model_settings['desired_samples']
        ...
        for i in xrange(offset, offset + sample_count):
            ...
            outputs = sess.run(self.mfcc_, feed_dict=input_dict).flatten()
            inputs = outputs
            ...
            if use_background and np.random.uniform(0, 1) < background_frequency:
                ...
                input_dict[self.background_data_placeholder_] = background_reshaped
                input_dict[self.background_volume_placeholder_] = background_volume
                inputs = sess.run(self.mfcc_, feed_dict=input_dict).flatten()

            # Run the graph to produce the output audio.
            data[i - offset, :] = inputs
            targets[i - offset, :] = outputs
            if close_sess:
                sess.close()
        return data, targets
